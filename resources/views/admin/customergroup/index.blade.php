    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card shadow mb-4">
             <div class="card-header">
                ...
                <a href="{{ route('admin.customergroup.add') }}" class="dt-button create-new btn btn-primary float-end">
            ...
            <div class=" text-nowrap dataTables_wrapper ">
                    ...
                    <tbody class="table-border-bottom-0">
                        @if ($customergroups->isNotEmpty())
                            @foreach ($customergroups as $key => $value)
                                <tr>
                                    <td>{{ $customergroups->firstItem() + $key }}</td>
                                    <td>{{ $value['title'] }}</td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.customergroup.add', encrypt($value['id'])) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ translate('Edit') }}</a>
                                                <a class="dropdown-item confirm-delete" href="javascript:void(0);"
                                                    data-href="{{ route('admin.customergroup.delete', encrypt($value['id'])) }}"><i
                ...
                <div class="row">
                    <div class="dataTables_paginate paging_simple_numbers mt-4">
                        {{ $customergroups->appends(request()->query())->links('pagination::bootstrap-5') }}
                    </div>
