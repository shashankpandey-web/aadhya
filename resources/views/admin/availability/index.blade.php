    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card shadow mb-4">
            <div class="card-header">
                ...
                <!-- <a href="{{ route('admin.availabilities.add') }}" class="dt-button create-new btn btn-primary float-end">
                    ...
                    <tbody class="table-border-bottom-0">
                        @if ($availabilities->isNotEmpty())
                            @foreach ($availabilities as $key => $availability)
                                <tr>
                                    <td>{{ $availabilities->firstItem() + $key }}</td>
                                    <td>{{ $availability->title}}</td>
                                    <td>{!! checkStatus($availability->status) !!}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.availabilities.add', encrypt($availability->id)) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ translate('Edit') }}</a>
                                                <!-- <a class="dropdown-item confirm-delete" href="javascript:void(0);"
                                                    data-href="{{ route('admin.availabilities.delete', encrypt($availability->id)) }}">
                        ...
                        @else
                            <tr>
                                <td colspan="4" class="text-center">{{ translate('No record Found') }}</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
