        ...
        <div class="card mb-4">
            <div class="card-widget-separator-wrapper">
                <div class="card-body card-widget-separator">
                    <form action="{{ route('admin.customersupports') }}" method="post" id="product_filter" class="">
                        @csrf
                        <div class="row gy-4 gy-sm-1">
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    <label class="col-form-label" for="status">{{ translate('Customer') }}</label>
                                    <select id="status" name="filter_customersupport_customer_id" class="form-select single-select" data-allow-clear="true">
                                        <option value="">Select Customer</option>
                                        <?php foreach ($customers as $key => $value) { ?>
                                                <option value="{{ $value->id }}" {{ getSelected($value->id, $common['filter_customersupport_customer_id']) }}>{{ $value->full_name }}</option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <select id="status" name="filter_customersupport_status" class="form-select single-select" data-allow-clear="true">
                                        <option value=""  > {{ translate('Select Status') }}</option>
                                        <?php foreach ($get_status as $key => $value) { ?>
                                            <option value="{{ $value }}" {{ getSelected($value, $common['filter_customersupport_status']) }}>{{ $value }}</option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
            ...
            <div class=" text-nowrap dataTables_wrapper ">
                <table class="table table-striped table-hover">
                    ...
                    <tbody class="table-border-bottom-0">
                        @if ($customersupports->isNotEmpty())
                            @foreach ($customersupports as $key => $value)
                                <tr>
                                    <td>{{ $customersupports->firstItem() + $key }}</td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.customersupports.view', encrypt($value['id'])) }}"><i
                ...
                <div class="row">
                    <div class="dataTables_paginate paging_simple_numbers mt-4">
                        {{ $customersupports->appends(request()->query())->links('pagination::bootstrap-5') }}
                    </div>
