    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            <div class="card-widget-separator-wrapper">
                <div class="card-body card-widget-separator">
                    <form action="{{ route('admin.tarots') }}" method="post" id="product_filter" class="">
                        @csrf
                        <div class="row gy-4 gy-sm-1">
                            <div class="col-md-3">
                                <div class="form-group mb-3 ">
                                    <label class="col-form-label">{{ translate('Title') }}</label>
                                    <input class="form-control" name="filter_tarot_title" type="text" value="{{ $common['filter_tarot_title']  }}" placeholder="{{ translate('Enter title') }}">
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <select id="status" name="filter_tarot_status" class="form-select single-select" data-allow-clear="true">
                                        <option value="Active" {{ getSelected('Active', $common['filter_tarot_status']) }}> {{ translate('Active') }}</option>
                                        <option value="Deactive" {{ getSelected('Deactive', $common['filter_tarot_status']) }}>{{ translate('Deactive') }}</option>
                                    </select>
                                </div>
                            </div>
        ...
        <div class="card shadow mb-4">
             <div class="card-header">
                ...
                <a href="{{ route('admin.tarots.add') }}" class="dt-button create-new btn btn-primary float-end">
            ...
            <div class=" text-nowrap dataTables_wrapper ">
                <table class="table table-striped table-hover">
                    ...
                    <tbody class="table-border-bottom-0">
                        @if ($tarots->isNotEmpty())
                            @foreach ($tarots as $key => $value)
                                <tr>
                                    <td>{{ $tarots->firstItem() + $key }}</td>
                                    <td>{{ $value['title'] }}</td>
                ...
                <div class="row">
                    <div class="dataTables_paginate paging_simple_numbers mt-4">
                        {{ $tarots->appends(request()->query())->links('pagination::bootstrap-5') }}
                    </div>
